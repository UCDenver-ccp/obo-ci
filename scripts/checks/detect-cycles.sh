...
if [[ -z ${ONT_FILE} || -z ${MAVEN} || -z ${LOG_FILE} ]]; then
...
${MAVEN} -e -f scripts/checks/pom-cycle-detection.xml exec:exec \
...
${MAVEN} -e -f scripts/checks/pom-cycle-detection-pair.xml exec:exec \
        -DontologyFile=${ONT_FILE} \
