...
while getopts "b:i:x:m:s:a:n:e:y:t:z:l:h" OPTION; do
    case ${OPTION} in
        # The work directory
...
if [[ -z ${XTRA_ONT_ID} ]]; then
    LOG_FILE=${LOG_DIRECTORY}/${ONT_ID}_cycles.log
    STATUS_FILE="${ONT_ID}_cycles.json"
    HEADER_JOB_NAME="${HEADER_JOB_NAME}_${ONT_ID}_cycles"
    ### add the header to the script file if one has been specified
    . ${CODE_BASE_DIRECTORY}/scripts/util/handle_header.bash

    printf "###\n### This script will detect cycles in ${owl_file}, using the OWLTools library.\n###\n" >> ${SCRIPT_FILE}
    ...
    printf "\n### update the cycles status to 'timeout', this way if the job does not finish it will be logged appropriately." >> ${SCRIPT_FILE}
    printf "\nsed -i 's/\\\"cycles\\\": null/\\\"cycles\\\": \\\"timeout\\\"/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
    # populate the template json with the path to the cycles log file
    pattern="[/]"
    escaped_log_file="${LOG_FILE//$pattern/\/}"
    printf "\n\n### log the location of the reasoner log file" >> ${SCRIPT_FILE}
    printf "\nsed -i 's/\\\"cycles_log\\\": null/\\\"cycles_log\\\": \\\"'\"${escaped_log_file}\"'\\\"/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
    printf "\n\n### check for cycles and log the output" >> ${SCRIPT_FILE}
    printf "\nprintf \"checking for cycles in ${owl_file}...\"" >> ${SCRIPT_FILE}
    printf "\n> ${LOG_FILE}" >> ${SCRIPT_FILE}
    printf "\n${CODE_BASE_DIRECTORY}/scripts/checks/detect-cycles.sh -i ${owl_file} -m ${MAVEN} -g ${LOG_FILE}" >> ${SCRIPT_FILE}
else
    LOG_FILE="${LOG_DIRECTORY}/${ONT_ID}+${XTRA_ONT_ID}_cycles.log"
    STATUS_FILE="${ONT_ID}+${XTRA_ONT_ID}_cycles.json"
    xtra_dir=$(echo "${BASE_DIRECTORY}/ontologies/${XTRA_ONT_ID}" | sed 's/\/\//\//g')
    xtra_owl_file="${xtra_dir}/${XTRA_ONT_ID}_flat.owl"

    HEADER_JOB_NAME="${HEADER_JOB_NAME}_${ONT_ID}+${XTRA_ONT_ID}_cycles"
    ### add the header to the script file if one has been specified
    . ${CODE_BASE_DIRECTORY}/scripts/util/handle_header.bash

    printf "###\n### This script will detect cycles over the merged ontologies in ${owl_file} and ${xtra_owl_file}, using the OWLTools library.\n###\n" >> ${SCRIPT_FILE}
    printf "\n### create the status file by copying from the template status file" >> ${SCRIPT_FILE}
    printf "\ncp ${CODE_BASE_DIRECTORY}/scripts/template.json ${STATUS_DIR}/${STATUS_FILE}" >> ${SCRIPT_FILE}
    printf "\n### update the id field in the status file" >> ${SCRIPT_FILE}
    printf "\nsed -i 's/\\\"id\\\": null,/\\\"id\\\": \\\"'\"${ONT_ID}+${XTRA_ONT_ID}\"'\\\",/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
    printf "\n### update the cycle detection status as 'timeout', this way if the job does not finish it will be logged appropriately." >> ${SCRIPT_FILE}
    printf "\nsed -i 's/\\\"cycles\\\": null/\\\"cycles\\\": \\\"timeout\\\"/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
    pattern="[/]"
    escaped_log_file="${LOG_FILE//$pattern/\/}"
    printf "\n\n### log the location of the reasoner log file" >> ${SCRIPT_FILE}
    printf "\nsed -i 's/\\\"cycles_log\\\": null/\\\"cycles_log\\\": \\\"'\"${escaped_log_file}\"'\\\"/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
    printf "\n\n### check for cycles and log the output" >> ${SCRIPT_FILE}
    printf "\nprintf \"checking for cycles in ${owl_file} + ${xtra_owl_file}...\"" >> ${SCRIPT_FILE}
    printf "\n> ${LOG_FILE}" >> ${SCRIPT_FILE}
    printf "\n${CODE_BASE_DIRECTORY}/scripts/checks/detect-cycles.sh -i ${owl_file} -x ${xtra_owl_file} -m ${MAVEN} -g ${LOG_FILE}" >> ${SCRIPT_FILE}
fi
printf "\ne=\$?" >> ${SCRIPT_FILE}

printf "\n### if the cycle detection succeeded then check the log file to see if any cycles were detected." >> ${SCRIPT_FILE}
printf "\nif [ \${e} == 0 ]; then" >> ${SCRIPT_FILE}
printf "\n\tif grep -q 'Found cycles in the graph' ${LOG_FILE}; then" >> ${SCRIPT_FILE}
printf "\n\t\tsed -i 's/\\\"cycles\\\": \\\"timeout\\\"/\\\"cycles\\\": true/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
printf "\n\telse" >> ${SCRIPT_FILE}
printf "\n\t\tsed -i 's/\\\"cycles\\\": \\\"timeout\\\"/\\\"cycles\\\": false/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
printf "\n\tfi" >> ${SCRIPT_FILE}
printf "\n\t### compute the number of cycles observed by counting lines in the log file that start with 'Cycle: '" >> ${SCRIPT_FILE}
printf "\n\tcnt=\$(grep -e '^\\[INFO\\] Cycle: ' ${LOG_FILE} | wc -l)" >> ${SCRIPT_FILE}
printf "\n\tsed -i 's/\\\"cycle_count\\\": null/\\\"cycle_count\\\": '\${cnt}'/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
printf "\nelse" >> ${SCRIPT_FILE}
printf "\n\t### The cycle detection process failed. Check the log to see why it failed." >> ${SCRIPT_FILE}
printf "\n\tsed -i 's/\\\"cycles\\\": \\\"timeout\\\"/\\\"cycles\\\": \\\"fail\\\"/' \"${STATUS_DIR}/${STATUS_FILE}\"" >> ${SCRIPT_FILE}
printf "\nfi" >> ${SCRIPT_FILE}
