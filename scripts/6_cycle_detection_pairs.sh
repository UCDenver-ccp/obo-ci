...
for index in ${!ID1s[*]}; do
  id1=${ID1s[$index]}
  id2=${ID2s[$index]}
  SCRIPT_FILE="${SCRIPT_DIRECTORY_CHECKS_PAIRS}/${id1}_${id2}.cycle.sh"
  if [[ -z ${HEADER_FILE} ]]; then
    ${CODE_BASE_DIRECTORY}/scripts/checks/cycle-detection-script-gen.sh -b ${BASE_DIRECTORY} -m ${MAVEN} -i ${id1} -x ${id2} -s ${STATUS_DIRECTORY_PAIRS} -t ${SCRIPT_FILE} -z ${CODE_BASE_DIRECTORY} -l ${LOG_DIRECTORY_CHECKS_PAIRS}
  else
    ${CODE_BASE_DIRECTORY}/scripts/checks/cycle-detection-script-gen.sh -b ${BASE_DIRECTORY} -m ${MAVEN} -i ${id1} -x ${id2} -s ${STATUS_DIRECTORY_PAIRS} -t ${SCRIPT_FILE} -a ${HEADER_FILE} -n ${HEADER_JOB_NAME} -e ${HEADER_EMAIL} -y ${HEADER_JOB_LOG_DIRECTORY} -z ${CODE_BASE_DIRECTORY} -l ${LOG_DIRECTORY_CHECKS_PAIRS}
  fi
