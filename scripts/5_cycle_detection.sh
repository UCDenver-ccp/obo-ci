...
for index in ${!IDs[*]}; do
  ...
  if [[ -z ${HEADER_FILE} ]]; then
    ${CODE_BASE_DIRECTORY}/scripts/checks/cycle-detection-script-gen.sh -b ${BASE_DIRECTORY} -m ${MAVEN} -i ${id} -s ${STATUS_DIRECTORY_INDIVIDUAL} -t ${SCRIPT_FILE} -z ${CODE_BASE_DIRECTORY} -l ${LOG_DIRECTORY_CHECKS}
  else
