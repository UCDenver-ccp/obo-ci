...
echo "Querying for incoherent classes in: ${ONT_FILE}" | tee -a ${LOG_FILE}
${MAVEN} -e -f scripts/classify/pom-incoherent-query-remove-abox.xml exec:exec \
        -DontologyFile=${ONT_FILE} \
        -DreasonerName=${REASONER_NAME} \
        -DoutputFile=${OUTPUT_FILE} \
        -DlaunchDir=${PATH_TO_ME} 2>&1 | tee -a ${LOG_FILE}
else
echo "Querying for incoherent classes in: ${ONT_FILE} ${XTRA_ONT_FILE}" | tee -a ${LOG_FILE}
${MAVEN} -e -f scripts/classify/pom-incoherent-query-pair-remove-abox.xml exec:exec \
        -DontologyFile=${ONT_FILE} \
