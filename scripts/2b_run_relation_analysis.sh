...
for index in ${!IDs[*]}; do
  ...
  if [[ -z ${HEADER_FILE} ]]; then
    ${CODE_BASE_DIRECTORY}/scripts/relation_analyses/relation-extraction-script-gen.sh -b ${BASE_DIRECTORY} -m ${MAVEN} -i ${id} -t ${SCRIPT_FILE} -z ${CODE_BASE_DIRECTORY} -l ${LOG_DIRECTORY_RELATION_EXTRACTION}
  else
    ${CODE_BASE_DIRECTORY}/scripts/relation_analyses/relation-extraction-script-gen.sh -b ${BASE_DIRECTORY} -m ${MAVEN} -i ${id} -t ${SCRIPT_FILE} -a ${HEADER_FILE} -n ${HEADER_JOB_NAME} -e ${HEADER_EMAIL} -y ${HEADER_JOB_LOG_DIRECTORY} -z ${CODE_BASE_DIRECTORY} -l ${LOG_DIRECTORY_RELATION_EXTRACTION}
  fi
